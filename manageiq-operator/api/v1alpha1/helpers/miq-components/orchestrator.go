...
func updateOrchestratorEnv(cr *miqv1alpha1.ManageIQ, c *corev1.Container) {
	...
	if cr.Spec.BaseWorkerImage == "" || cr.Spec.WebserverWorkerImage == "" || cr.Spec.UIWorkerImage == "" {
		...
		string2 := strings.Join(strings.Split(string1[0], "/")[:len(strings.Split(string1[0], "/"))-1], "/")
		c.Env = addOrUpdateEnvVar(c.Env, corev1.EnvVar{Name: "CONTAINER_IMAGE_NAMESPACE", Value: string2})
